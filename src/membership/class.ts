...
export class MembershipManager {
  ...
  private setupExpiredRoleChecks() {
    ...
    memberships.forEach((membership) => {
      if (membership.expiresAt && membership.expiresAt > now) {
        ...
        console.log(
          `Scheduling role removal for ${membership.userId} in ${delay}ms`,
        );

        setTimeout(
          () => removeRole(guild, membership.userId, membership.roleId),
          delay,
        );
      }
    });
  ...
  public async assignMembership(interaction: ChatInputCommandInteraction) {
    ...
    if (!targetUser || !role || !duration) {
      return interaction.reply({
        content: 'Invalid user, role, or duration.',
        ephemeral: true,
      });
    }
    ...
    if (targetUser.roles.cache.has(role.id)) {
      return interaction.reply({
        content: `${targetUser} already has this role.`,
        ephemeral: true,
      });
    }
    ...
    try {
      ...
      if (duration.toLowerCase() !== 'perm') {
        ...
        setTimeout(
          () => removeRole(interaction.guild!, targetUser.id, role.id),
          timeMs,
        );
      }
    } catch (error) {
  ...
  public async removeMembership(interaction: ChatInputCommandInteraction) {
    ...
    if (!targetUser.roles.cache.has(role.id)) {
      return interaction.reply({
        content: `${targetUser} does not have this role.`,
        ephemeral: true,
      });
    }
  ...
  private async logMembershipChange(
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: 'User',
          value: `${targetUser} (${targetUser.user.tag})`,
          inline: true,
        },
    ...
    if (duration && action === 'added') {
      embed.addFields({
        name: 'Duration',
        value: duration === 'perm' ? 'Permanent' : duration,
        inline: true,
      });
    }
